...
# 분리해뒀다. *주의 : sms_spam_model.pkl과 항상 같은 폴더에 이 파일도 있어야 한다.

# 배경: char n-gram(2~6) 벡터라이저에 URL을 그대로 넣으면 "http", "bit.ly"
# 같은 문자열이 "tt", "it", "tp:/"처럼 의미 없는 조각으로 쪼개져서, 모델
# 설명(top_features)에 노출됐을 때 사람이 읽기 어려운 문제가 있었다. (7월 16일 분석 결과.)
# URL을 하나의 'url' placeholder 토큰으로 치환해서, 이 문제를 줄인다.
# 이런 처리를 하면 도메인명 자체(예: "복권천재.kr")가 가진 판별력은 사라지고
# "URL이 있다/없다"는 정보만 남는다. 문제가 되는 도메인 주소를
# 굳이 사용자에게 노출하지 않아도 되니 오히려 더 안전하다고 생각된다.

#url패턴 사용을 위한 정규식 import
import re

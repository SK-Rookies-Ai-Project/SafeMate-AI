...
def main():
    ...
    second_bundle = (
        ModelBundle.load(args.second_model)
        if args.second_model.is_file()
        else None
    )
    classes = sorted(char_bundle.label_encoder.classes_)
    if second_bundle is None:
        print(
            f"단일 모델 로딩 완료: {args.char_model.name}"
            f"({char_bundle.model_type})"
        )
        print(
            f"선택 모델이 없어 앙상블을 생략합니다: {args.second_model}"
        )
    else:

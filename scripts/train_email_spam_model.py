...
#
# 실행 예시:
# python train_email_spam_model.py
# python train_email_spam_model.py --data_path data/raw/korean_spam_ham_binary_dataset_6272row.csv --model_path models/email_spam_logistic_tfidf.pkl
# ============================================================
...
# 3. 이메일 데이터 추출 함수
# 입력값 : 전체 원본 DataFrame
# 반환값 : 이메일 데이터 DataFrame(label, text)

def extract_email_data(data):
    # channel이 이메일인 행만 추출하고 label, text 컬럼만 선택(loc, copy)
    email_data = data.loc[
...
def print_data_summary(email_data):
    ...
    print('label + text 중복 개수:', email_data.duplicated(subset=['label', 'text']).sum())
    print('-' * 60)

    # 정상/스팸 라벨 분포 확인(value_counts)
    print('[전처리 전 라벨 분포]')
    print(email_data['label'].value_counts().sort_index())
    print('-' * 60)

    # 이메일 문자열 길이 분포 확인(str.len, describe)
    text_length = email_data['text'].astype(str).str.len()
    print('[이메일 문자열 길이 통계]')
...
def preprocess_email_data(email_data):
    # label 또는 text가 없는 행 제거(dropna)
    email_data = email_data.dropna(
        subset=['label', 'text']
    ).copy()

    # text를 문자열로 변환하고 여러 공백과 줄바꿈을 공백 한 칸으로 정리(str.replace, strip)
    email_data['text'] = (
        email_data['text']
        .astype(str)
    ...
    # label과 text가 완전히 같은 중복 행 제거(drop_duplicates)
    email_data = (
        email_data
...
# 6. 입력 데이터와 정답 데이터 분리 함수
# 입력값 : 전처리된 이메일 데이터 DataFrame
# 반환값 : X(text), y(label)

def split_feature_target(email_data):
    # X는 모델에 입력할 이메일 문자열
    X = email_data['text']
...
def split_train_test(X, y):
    # stratify=y -> 학습/테스트 데이터의 정상·스팸 비율을 비슷하게 유지
...
def test_sample_emails(model):
    ...
    sample_probabilities = model.predict_proba(sample_emails)[:, 1]
    ...
    for text, prediction, probability in zip(
        ...
        print('-' * 60)
        print('입력:', text)
        print('예측:', '스팸' if prediction == 1 else '비스팸')
        print('스팸 확률:', round(float(probability), 4))
...
def check_saved_model(model_path):
    ...
    prediction = loaded_model.predict(test_email)[0]
    spam_probability = loaded_model.predict_proba(test_email)[0][1]

    print('[저장된 모델 예측 확인]')
    print('입력:', test_email[0])
    print('예측 결과:', '스팸' if prediction == 1 else '비스팸')
    print('스팸 확률:', round(float(spam_probability), 4))
    print('-' * 60)
...
def main():
    ...
    train_and_save_final_model(X, y, args.model_path)

    # 13. 저장된 모델이 정상적으로 불러와지는지 확인
    check_saved_model(args.model_path)
